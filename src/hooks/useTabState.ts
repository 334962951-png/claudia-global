...
/**
 * Custom hook for managing application tab state
 *
 * Provides a comprehensive interface for managing tabs in the application,
 * including creating, updating, removing, and navigating between tabs.
 ...
 * ```tsx
 * function TabManager() {
 *   const { tabs, activeTabId, addTab, removeTab, setActiveTab } = useTabState();
 *
...
export const useTabState = (): UseTabStateReturn => {
  ...
  const createChatTab = useCallback(
    (projectId?: string, title?: string): string => {
  ...
  const createAgentExecutionTab = useCallback(
    (agent: { name: string; [key: string]: unknown }, _tabId: string): string => {
      return addTab({
