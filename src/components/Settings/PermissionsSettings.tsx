...
export const PermissionsSettingsSection: React.FC<PermissionsSettingsSectionProps> = ({
  allowRules,
  denyRules,
  addPermissionRule,
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold mb-2">{t.settings.permissionRules}</h3>
        <p className="text-sm text-muted-foreground mb-4">{t.settings.permissionRulesDesc}</p>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {allowRules.length === 0 ? (
            <p className="text-xs text-muted-foreground py-2">{t.settings.noAllowRules}</p>
          ) : (
            allowRules.map((rule) => (
              <motion.div
                key={rule.id}
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {denyRules.length === 0 ? (
            <p className="text-xs text-muted-foreground py-2">{t.settings.noDenyRules}</p>
          ) : (
            denyRules.map((rule) => (
              <motion.div
                key={rule.id}
      ...
      <div className="pt-2 space-y-2">
        <p className="text-xs text-muted-foreground"><strong>{t.settings.examples}</strong></p>
