...
export function SessionOutputViewer({ session, onClose, className }: SessionOutputViewerProps) {
  ...
  const handleCopyAsMarkdown = async () => {
    ...
    for (const msg of messages) {
      if (msg.type === "system" && msg.subtype === "init") {
        ...
        markdown += `## User\n\n`;
        for (const content of msg.message.content || []) {
          const contentObj = content as { type?: string; text?: string; content?: string };
          if (contentObj.type === "text") {
            markdown += `${contentObj.text}\n\n`;
          } else if (contentObj.type === "tool_result") {
  ...
  useEffect(() => {
    ...
    setupLiveEventListeners();
  }, [session.id, session.session_id, getCachedOutput, loadOutput, setupLiveEventListeners]);
                  ...
                  if (
                    ...
                    const toolUse = prevMsg.message.content.find(
                      (c: unknown) =>
                        ...
                        (c as Record<string, unknown>).id === contentObj.tool_use_id
                    );
                    if (toolUse) {
